...
$(function() {
    $('#function_form_edit').form({
        onLoadSuccess : function(data) { // 加载完毕后在格式化表单
            ...
            $('#functionAccessChannel_edit').val(data.functionAccessChannel);
            ...
            // 访问 途径
            $('#functionAccessChannel_edit').combobox({
                required : true,
                method : 'get',
                url : basePath + 'static/llsfw/json/functionAccessChannel.json',
                valueField : 'value',
                textField : 'text',
                editable : false
            });
            ...
            // 功能图标
            $('#icon_edit').validatebox({
                required : false,
                validType : [ "length[0,100]", "not_chinese" ]
            });
